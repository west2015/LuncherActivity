 ...
 */
public class CouponDetailBuyActivity extends ModelActivity implements View.OnClickListener,OnAsyncTaskUpdateListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.coupon_detail_buy_layout);
        find();
    }
    private void find() {
        ...
        tvSueDate = (TextView)findViewById(R.id.tvSueDate);
...
//                	Toast.makeText(CouponDetailGetActivity.this, "Number = " + coupon.getLimitNum(), Toast.LENGTH_SHORT).show();
                	if(coupon.getIsLimit()==limitTrue && buyCount>=coupon.getLimitNum()){
                        Toast.makeText(CouponDetailBuyActivity.this, 
                        		"对不起，每个账号限领"+coupon.getLimitNum()+"张！", Toast.LENGTH_SHORT).show();
                        return;
                	}
                	if(buyCount>=coupon.getCirculation()){
                        Toast.makeText(CouponDetailBuyActivity.this, "库存不够", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    tvCount.setText(String.valueOf(buyCount + 1));
                }
                else{
                    Toast.makeText(CouponDetailBuyActivity.this, "暂无库存", Toast.LENGTH_SHORT).show();
                }
                break;
            ...
            case R.id.ll_buy_now:   //立即购买
                ...
                if(nowCount==0){
                    Toast.makeText(CouponDetailBuyActivity.this,"请选择购买量",Toast.LENGTH_SHORT).show();
                    return;
                }
                else
               	if(MainApplication.getInstance().getMember()==null){
                    new startIntent(CouponDetailBuyActivity.this,LoginActivity.class);
                    return;
                }
               	else {
                    ...
                    new startIntent(CouponDetailBuyActivity.this,CouponFormActivity.class,bundleForm);
                }
        }
    }
    @Override
