...
public class LoginPost {
    ...
    public LoginPost(Context context, Member member) {
        if(!AndroidServerFactory.PRODUCTION_MODEL)
            MobclickAgent.onEvent(context, "path", "/api/v1/member/login");
    ...
    class DownloadTask extends AsyncTaskEx<Void, Void,MemberRespondEntity> {
        ...
        protected MemberRespondEntity doInBackground(Void... params) {
            ...
            try {
                status = AndroidServerFactory.getServer().postLogin(member);
            } catch (IOException e) {
                e.printStackTrace();
        ...
        protected void onPostExecute(MemberRespondEntity result) {
            ...
            if(result!=null && result.getDatas()!=null && result.getStatus().getSuccess().equals("true")){
                ...
                if(BaidupushUtils.hasBind(context)){
                    new BaiduPushMsgPost(context,result.getDatas().getId(),new DbBaiduPush(context).getBaiduBindingMsg()).setListener(listener);
                }
            }
