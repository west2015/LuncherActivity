 ...
 */
public class FormDetailDeleteGet {
    private OnAsyncTaskUpdateListener listener;
    private String message;
    private Context context;
    private Integer[] ids;

    public FormDetailDeleteGet(Context context,Integer[] ids) {
        if(!AndroidServerFactory.PRODUCTION_MODEL)
            MobclickAgent.onEvent(context, "path", "/api/v1/order/delProducts");
    ...
    class DownloadTask extends AsyncTaskEx<Void, Void,Object> {
        ...
        protected Object doInBackground(Void... params) {
            Object status=null;
            try {
                status = AndroidServerFactory.getServer().getFormDetailDelete(ids);
            } catch (IOException e) {
                e.printStackTrace();
