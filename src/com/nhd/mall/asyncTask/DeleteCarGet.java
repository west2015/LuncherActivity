 ...
 */
public class DeleteCarGet {
    private OnAsyncTaskUpdateListener listener;
    private String message;
    private Context context;
    private Integer[] ids;

    public DeleteCarGet(Context context,Integer[] ids) {
        if(!AndroidServerFactory.PRODUCTION_MODEL)
            MobclickAgent.onEvent(context, "path", "/api/v1/car/del");
    ...
    class DownloadTask extends AsyncTaskEx<Void, Void,Object> {
        ...
        protected Object doInBackground(Void... params) {
            Object status=null;
            try {
                status = AndroidServerFactory.getServer().deleteCarGet(ids);
            } catch (IOException e) {
                e.printStackTrace();
