 ...
 */
public class FormDetailGet {
    private OnAsyncTaskUpdateListener listener;
    private String message;
    private Context context;
    private Integer id;

    public FormDetailGet(Context context,Integer id) {
        if(!AndroidServerFactory.PRODUCTION_MODEL)
            MobclickAgent.onEvent(context, "path", "/api/v1/order/detail");
    ...
    class DownloadTask extends AsyncTaskEx<Void, Void,Object> {
        ...
        protected Object doInBackground(Void... params) {
            Object status=null;
            try {
                status = AndroidServerFactory.getServer().getFormDetail(id);
            } catch (IOException e) {
                e.printStackTrace();
