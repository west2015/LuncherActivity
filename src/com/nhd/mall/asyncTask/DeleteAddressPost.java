 ...
 */
public class DeleteAddressPost {
    private OnAsyncTaskUpdateListener listener;
    private String message;
    private Context context;
    private Integer id;

    public DeleteAddressPost(Context context, Integer id) {
        if(!AndroidServerFactory.PRODUCTION_MODEL)
            MobclickAgent.onEvent(context, "path", "/api/v1/address/del");
    ...
    class DownloadTask extends AsyncTaskEx<Void, Void,Object> {
        ...
        protected Object doInBackground(Void... params) {
            Object status=null;
            try {
                status = AndroidServerFactory.getServer().getDeleteAddress(id);
            } catch (IOException e) {
                e.printStackTrace();
