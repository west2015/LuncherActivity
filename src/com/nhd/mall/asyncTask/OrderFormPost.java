...
public class OrderFormPost {
    ...
    public OrderFormPost(Context context,FormEntity form) {
        if(!AndroidServerFactory.PRODUCTION_MODEL)
            MobclickAgent.onEvent(context, "path", "/api/v1/order/add");
    ...
    class DownloadTask extends AsyncTaskEx<Void, Void,Object> {
        ...
        protected Object doInBackground(Void... params) {
            Object status=null;
            try {
                status = AndroidServerFactory.getServer().postOrderForm(form);
            } catch (IOException e) {
                e.printStackTrace();
